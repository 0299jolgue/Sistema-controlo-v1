...
def page(request:Request,name,**ctx):
...
async def index(request:Request):return RedirectResponse("/dashboard" if request.session.get("admin") else "/login",303)
@router.get("/login",response_class=HTMLResponse)
async def login_page(request:Request):return page(request,"login.html",error=None)
@router.post("/login")
...
async def dashboard(request:Request,db:Session=Depends(get_db)):guard(request);seed_demo(db);return page(request,"dashboard.html",devices=db.query(Device).all(),recent=db.query(LogEvent).order_by(LogEvent.created_at.desc()).limit(8).all())
@router.get("/devices",response_class=HTMLResponse)
async def devices(request:Request,db:Session=Depends(get_db)):guard(request);seed_demo(db);return page(request,"devices.html",devices=db.query(Device).all(),enrollments=db.query(EnrollmentToken).order_by(EnrollmentToken.created_at.desc()).limit(10).all())
@router.post("/devices/enrollment")
async def enrollment(request:Request,db:Session=Depends(get_db)):guard(request);issue_enrollment(db);add_log(db,"enrollment","Nova inscrição criada.");return RedirectResponse("/devices",303)
@router.post("/devices/{device_id}/rename")
async def rename(request:Request,device_id:int,name:str=Form(...),db:Session=Depends(get_db)):guard(request);d=db.get(Device,device_id);d.name=name.strip()[:120] or d.name;db.commit();add_log(db,"device_renamed",f"Nome alterado para {d.name}",d.id);return RedirectResponse(f"/devices/{device_id}",303)
@router.post("/devices/{device_id}/revoke")
async def revoke(request:Request,device_id:int,db:Session=Depends(get_db)):guard(request);d=db.get(Device,device_id);d.authorized=False;d.online=False;db.commit();add_log(db,"revocation",f"Acesso revogado: {d.name}",d.id,"warning");return RedirectResponse("/devices",303)
@router.get("/devices/{device_id}",response_class=HTMLResponse)
async def device(request:Request,device_id:int,db:Session=Depends(get_db)):guard(request);d=db.get(Device,device_id);return page(request,"device.html",device=d,commands=db.query(Command).filter_by(device_id=device_id).order_by(Command.created_at.desc()).limit(20).all(),messages=db.query(Message).filter_by(device_id=device_id).order_by(Message.created_at.desc()).limit(20).all(),logs=db.query(LogEvent).filter_by(device_id=device_id).order_by(LogEvent.created_at.desc()).limit(30).all())
...
async def logs(request:Request,db:Session=Depends(get_db)):guard(request);return page(request,"logs.html",logs=db.query(LogEvent).order_by(LogEvent.created_at.desc()).limit(300).all())
@router.get("/apk-builder",response_class=HTMLResponse)
async def builder(request:Request):guard(request);return page(request,"apk_builder.html")
@router.post("/apk-builder")
async def build(request:Request,name:str=Form("Android Agent"),version:str=Form("1.0.0"),db:Session=Depends(get_db)):guard(request);db.add(ApkBuild(name=name[:120],version=version[:60],status="Completed",artifact_path="android/app/build/outputs/apk/debug/app-debug.apk"));db.commit();return RedirectResponse("/apks",303)
@router.get("/apks",response_class=HTMLResponse)
async def apks(request:Request,db:Session=Depends(get_db)):guard(request);return page(request,"apks.html",builds=db.query(ApkBuild).order_by(ApkBuild.created_at.desc()).all())
@router.get("/settings",response_class=HTMLResponse)
async def settings_page(request:Request,db:Session=Depends(get_db)):guard(request);return page(request,"settings.html",values={x.key:x.value for x in db.query(AppSetting).all()})
@router.post("/settings")
async def settings_save(request:Request,platform_name:str=Form("Sistema Controlo"),server_url:str=Form(""),db:Session=Depends(get_db)):
 guard(request)
